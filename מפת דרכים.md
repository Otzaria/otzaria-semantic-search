...
| P1 — הקשחת השלד | תיקון fmt/clippy, semantic-only, filters, re-index, manifest corruption/mismatch, reopen tests; הסרת embedding מזויף מ־production |
| P2 — Inference אמיתי | backend trait; השוואת פלט מול Python reference; tokenizer, EOS, last-token ו־L2 זהים; batch inference אמיתי |
...
2. **Real inference spike**  
   הפקת golden vectors מהמודל האמיתי והשוואת Candle/llama.cpp. לא לגעת עדיין ב־UI או ב־fusion tuning.

3. **Storage-and-scale prototype**  
