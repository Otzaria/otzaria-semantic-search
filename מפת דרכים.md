> **מסמך היסטורי.** זו הסקירה שנכתבה לפני PR #1–#3, והיא נשמרת כתיעוד של ההנמקה
> וההחלטות באותו זמן. **סדר העבודה המחייב הוא [`שלבי ויעדי התקדמות.md`](שלבי%20ויעדי%20התקדמות.md)**,
> והיקף המוצר מוגדר ב־[`docs/PRODUCT_CONTRACT.md`](docs/PRODUCT_CONTRACT.md).
>
> מה שהשתנה מאז ומופיע כאן כלא-מדויק:
>
> - „Inference אמיתי חסר לחלוטין” — **נסגר ב־PR #2**, מאחורי `--features llama-backend`.
> - סעיף 5 מציג `Mutable user-books overlay` בדיאגרמה, ו־P9 מזכיר „overlay קטן לספרי
>   משתמש”. **שני אלה בוטלו**: אין overlay, וספרים אישיים נשארים לקסיקליים בלבד.
> - P7 מזכיר progress stream ו־cancel/resume. **בוטלו**: אין אינדוקס באפליקציה.
> - „Vector DB מתמשך חסר לחלוטין” — PR #3 הוסיף `ZevcStore` שמתמיד ונפתח מחדש, ו־**S2a
>   חיבר אותו**: מסלול האפליקציה פותח ארטיפקט מותקן דרך `ReadOnlyZevcStore`. הוא עדיין
>   **אינו ANN ואינו mmap**, והפתיחה טוענת הכול ל־RAM — הפער שנשאר מתואר ב־S2b.
> - „`IndexVersion` אינו קושר את החבילה ל־corpus” — **נסגר ב־S3**: זהות
>   corpus/Tantivy/ID scheme/מודל/store, מאומתת לפני פתיחה. ראו
>   [`docs/ARTIFACT_CONTRACT.md`](docs/ARTIFACT_CONTRACT.md).
> - מספרי האימות בסעיף „תוצאות האימות” הם של אותה סקירה. המצב הנוכחי: `cargo fmt`,
>   clippy ו־361/383 בדיקות עוברים (רגיל / `mock-embedding`).
...
- [`EmbeddingRuntime::load`](src/semantic/embedding.rs#L43) בודק רק שהקובץ קיים. גם קובץ `GGUF_MOCK` עובר.
- [`embed_one`](src/semantic/embedding.rs#L68) מפעיל SHA-256 feature hashing, לא מודל.
- [`VectorStore`](src/semantic/store.rs#L73) הוא `HashMap` בזיכרון; הנתיב לדיסק רק נוצר כתיקייה.
- [`commit()`](src/semantic/store.rs#L265) הוא stub.
- ה־manifest נשמר אחרי אינדוקס, אך הווקטורים לא. לאחר הפעלה מחדש יכול להיות manifest שמצהיר “מאונדקס” ומאגר וקטורים ריק.
- ה־crate מוגדר רק כ־[`rlib`](Cargo.toml#L8), ללא `flutter_rust_bridge`.
- ה־API ב־[`hybrid_search.rs`](src/api/hybrid_search.rs#L15) אינו בר־שימוש אמיתי דרך FFI: אין factory מנתיבים, אין API לאינדוקס, אין progress/cancel, והבנאי מקבל `HybridCoordinator` פנימי שלא ניתן ליצור מ־Dart.
...
- לכן ה־CI המקומי אינו ירוק כרגע, משום שאותן בדיקות מופיעות ב־[workflow](.github/workflows/ci.yml#L32).
...
- `SemanticOnly` אינו מיושם: רק `LexicalOnly` מקבל טיפול מיוחד. כל מצב אחר מחזיר Hybrid וממשיך לכלול תוצאות לקסיקליות ב־[`coordinator.rs`](src/hybrid/coordinator.rs#L67).
...
כרגע ה־[`Chunker`](src/semantic/chunker.rs#L37) שולח למודל רק את גוף השורה, או שורות סמוכות כאשר היא קצרה. שם הספר ו־`reference` נשמרים כ־metadata בלבד.
...
בצילום מצב מקומי של הספרייה מופיעות כ־6,058,210 שורות ב־`otzaria/docs/institutional_edition_plan.md:74`.
...
- מנגנון הבנייה הרב־פלטפורמי כבר קיים ב־`otzaria_search_engine` (`rust/Cargo.toml:6`), כולל Cargokit ו־precompiled binaries.
...
| P3 — ייצוג שורה | ניסוי `line` מול `title+reference+line` ומול context שכנים; בחירת 1024/512/256/128 ממדים לפי איכות. **נמשך כ־S1** |
| P4 — Vector backend | backend מתמשך, read-only בזמן ריצה, filters, ו־ANN **רק אם מדידה מחייבת**; מבחן reopen/crash ומבחן מיליון/שישה מיליון רשומות. **נמשך כ־S2**. חלקית ב־PR #3: `ZevcStore` מתמיד ונפתח מחדש, אך אינו מחובר ל־engine ואינו ANN. `flush/WAL` ו־`delete/upsert` בזמן ריצה **אינם נדרשים** באינדקס read-only |
| P5 — Hybrid נכון | ✅ **בוצע בעיקרו** (PR #3): RRF בשימוש לפי פרופיל, thresholds, grouping, pagination. נשארו: hydration מ־Tantivy (באחריות `otzaria_search_engine`) ו־`total_count` אמיתי |
...
| P7 — API דרך FRB | `configureSemantic`, `semanticStatus`, `open`/`install_official_semantic_index`, וחיפוש מאוחד. ~~progress stream, cancel/resume~~ — **בוטלו** (אין אינדוקס באפליקציה) |
| P8 — אפליקציה | `RetrievalMode` חדש: lexical/hybrid/semantic, בנפרד מ־exact/advanced/fuzzy; מסך הורדת מודל וארטיפקט, מצב תקינות ושגיאות. ~~התקדמות אינדוקס~~ — **בוטל** |
| P9 — הפצה | official read-only index מוכן מראש; checksum/versioning והתאמה לגרסת הספרייה. ~~overlay קטן לספרי משתמש~~ — **בוטל** |
| P10 — release gates | בנייה בכל היעדים, בדיקות עם מודל אמיתי, latency/RAM/disk, power-loss recovery ו־lexical fallback |

> P6–P10 כאן מוחלפים ב־S4–S8 של [`שלבי ויעדי התקדמות.md`](שלבי%20ויעדי%20התקדמות.md),
> שמפרקים את אותה עבודה לפי מאגר ומוסיפים את חוזה הארטיפקט שחסר כאן.
...
3. **Storage-and-scale prototype** — ⏳ חלקית (PR #3), ופתוח כ־S2.
   טעינת מיליון רשומות סינתטיות, מדידת 128/256/512 ממדים וקוונטיזציה, ואז בחירת backend.
   PR #3 הוסיף את ה־store המתמיד ואת תשתית המדידה; **המדידה בקנה מידה טרם נעשתה**.

רק לאחר שלושת אלה נכון להתחיל את חיבור ה־FFI לאוצריא. אחרת ייבנה Bridge סביב חוזה ומנוע שעוד עומדים להשתנות.

הערת הפצה אחרונה: מאגר המודל דורש כרגע אישור תנאים ושיתוף פרטי קשר, וברישיון
non-commercial. **הוכרע ב־S0:** המודל בבעלות הפרויקט, ולכן אין כאן חסם זכויות; הוא
**אינו** מצורף למתקין ואינו תלות של המאגר. הפירוט וההמשך הפתוח:
[`docs/MODEL_DISTRIBUTION.md`](docs/MODEL_DISTRIBUTION.md).
[כרטיס המודל והרישיון](https://huggingface.co/EMD123/Otzaria-Embedding-V1-Flash-0.6B-GGUF)
