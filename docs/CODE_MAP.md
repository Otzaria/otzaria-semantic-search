...
* [`src/distribution/corpus.rs`](../src/distribution/corpus.rs) — **הפורט אל האינדקס
  ...
  - **שלוש תוצאות, וכולן העיקר:** זהות ה-corpus נקראת מהאינדקס ולא מוקלדת ליד
    ...
    מהראשון; ו-`expected_line_ids()` הופך „ארטיפקט מלא” לטענה שאפשר לבדוק. בלי השלישי,
    וקטור תקין אחד מתוך שישה מיליון היה מפיק „ארטיפקט רשמי” שכל ספירה, checksum וזהות
    מסכימים איתו.
  - **זו אינה „כל השורות באינדקס”:** מתכון ההטמעה מדלג על שורות קצרות מדי, וארטיפקט
    שדילג עליהן אינו חסר. לכן הקבוצה נענית בידי מי שמממש את המתכון. ל-`JsonlCorpus`:
    ייצאו בדיוק את השורות שאמורות לקבל וקטור.
...
* [`src/distribution/packer.rs`](../src/distribution/packer.rs) — **צד ה-build (S4a).**
  ...
  - `validate_artifact()` — האימות של הריצה (`verify_for_install`, פריסת payload, פתיחת
    ...
    אומר על השורה שלה, וקבוצת ה-IDs מושווית ל-`expected_line_ids()`. חשוף גם לבדו, ולכן
