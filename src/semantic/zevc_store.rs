...
impl ZevcStore {
    ...
    pub fn save_to_disk(&self) -> Result<(), VectorStoreError> {
        ...
        let mut v_file = BufWriter::new(File::create(&v_tmp).map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?);
        let mut m_file = BufWriter::new(File::create(&m_tmp).map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?);

        for (_, record) in &state.records {
            let meta_json = serde_json::to_string(&record.metadata).map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?;
            writeln!(m_file, "{}", meta_json).map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?;
            
            let mut bytes = Vec::with_capacity(record.vector.len() * 4);
            for f in &record.vector {
                bytes.extend_from_slice(&f.to_le_bytes());
            }
            v_file.write_all(&bytes).map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?;
        }

        let b_file = File::create(&b_tmp).map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?;
        serde_json::to_writer(b_file, &state.book_index).map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?;

        v_file.flush().map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?;
        m_file.flush().map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?;
        
        fs::rename(v_tmp, v_path).map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?;
        fs::rename(m_tmp, m_path).map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?;
        fs::rename(b_tmp, b_path).map_err(|e| VectorStoreError::CommitFailed { reason: e.to_string() })?;
    ...
    pub fn load_from_disk(&self) -> Result<(), VectorStoreError> {
        ...
        let m_file = File::open(&m_path).map_err(|e| VectorStoreError::OpenFailed { reason: e.to_string() })?;
        let mut v_file = File::open(&v_path).map_err(|e| VectorStoreError::OpenFailed { reason: e.to_string() })?;
        ...
        for line in m_reader.lines() {
            ...
            state.records.insert(metadata.semantic_id.clone(), StoredVectorRecord {
                metadata,
                vector,
            });
        }
        
        let b_file = File::open(&b_path).map_err(|e| VectorStoreError::OpenFailed { reason: e.to_string() })?;
...
impl VectorStoreBackend for ZevcStore {
    ...
    fn book_keys(&self) -> Vec<String> {
        self.state.read().unwrap().book_index.keys().cloned().collect()
    }
}
...
mod tests {
    ...
    fn test_zevc_store_crud_and_persistence() {
        ...
        let batch = vec![
            (sample_metadata("id1", "book1.txt"), vec![1.0, 0.0, 0.0, 0.0]),
            (sample_metadata("id2", "book1.txt"), vec![0.0, 1.0, 0.0, 0.0]),
        ];
        ...
        // Test clear
        store2.insert_batch(vec![(sample_metadata("id3", "b2"), vec![0.0,0.0,1.0,0.0])]).unwrap();
    ...
    fn test_corrupted_files() {
        ...
        // Corrupt vectors
        let mut v_file = fs::OpenOptions::new().write(true).open(store.vectors_path()).unwrap();
